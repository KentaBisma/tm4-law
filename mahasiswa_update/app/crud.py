...
def update_mahasiswa(db: Session, mhsw: schemas.MahasiswaUpdate):
    db_mhsw = db.query(models.Mahasiswa).filter(models.Mahasiswa.npm == mhsw.npm).first()
    db_mhsw.nama = mhsw.nama
    db.commit()
    db.refresh(db_mhsw)
    return db_mhsw

def delete_mahasiswa(db: Session, npm: int):
    db.delete(db.query(models.Mahasiswa).filter(models.Mahasiswa.npm == npm).first())
    db.commit()
